...
def test_pdf_reader_extract_text_success():
    ...
    # Mock do documento PDF retornado pelo pymupdf.open
    mock_doc = MagicMock()
    ...
    # e pymupdf.open para retornar o nosso mock_doc
    with patch.object(Path, "exists", return_value=True), \
         patch("pymupdf.open", return_value=mock_doc) as mock_open:
...
def test_pdf_reader_corrupted_file():
    ...
    # Forçamos o arquivo a "existir", mas a abertura pelo pymupdf falhará
    with patch.object(Path, "exists", return_value=True), \
         patch("pymupdf.open", side_effect=Exception("Formato inválido")):
