...
- [x] **Painel de Controle Desktop Nativo em PyQt6 (`painel.py` / `desktop/ui/pyqt_app.py`):** Painel desktop construído em **PyQt6 (Qt6 nativo)** com tema escuro moderno (QSS), alta estabilidade e layout otimizado em **2 Abas Estratégicas**:
  ...
  - **Aba 1 (Operação & Leitura em Split View):** Coluna esquerda fixa com seletor nativo de pastas (`QFileDialog`), botão principal de monitoramento, **Barras Lineares Tradicionais de Progresso** (Total da Pasta e Sessão Atual com porcentagens e contadores), **Caixa de Status Estática com Extração Inteligente do Número/Código do RELINT (`RELINT Nº ...` + Tooltip com nome completo)**, **Indicador de Leitura Ativa Estilo Web** (animação contínua a 60 FPS com spinner e marquee animado) e card de Ações Rápidas com alternador do Modo IA, botão do Dashboard Web e botão **`⛔ Encerrar Todos os Serviços & Sair`** posicionado estrategicamente abaixo do Dashboard. Coluna direita com **Console de logs com Efeito Typewriter Inteligente (Auto-Speed)** e auto-scroll thread-safe.
  - **Aba 2 (Serviços & Relatórios):** Card de diagnóstico dos serviços com **ServiceWatcher assíncrono em background** (zero bloqueios de rede/subprocessos na UI thread) e **histórico limpo de RELINTs processados** (apenas nome do arquivo, badge de método `IA`/`Regex`, badge de status `🟢 Lido com Sucesso` e **botão largo de reprocessamento `🔄 Re-processar`**).
  - **Performance & Zero Latência:** Fila de logs em lote com Typewriter a 60 FPS eliminando engasgos de renderização do `QTextEdit`, proteção seletiva de botões de disco durante leitura e atualização ultrarrápida de progresso (< 0.1ms).
