...
- [x] **Painel de Controle Desktop Nativo em PyQt6 (`painel.py` / `desktop/ui/pyqt_app.py`):** Painel desktop construído em **PyQt6 (Qt6 nativo)** com tema escuro moderno (QSS), alta estabilidade e sem quebras de API. Implementa seleção nativa de pastas (`QFileDialog`), barras de progresso gerais e de sessão, controle de ciclo de vida de serviços (FastAPI Backend e SvelteKit Frontend via subprocess), alternância do motor Ollama/LLM sob demanda com botão de teste de conexão, terminal de Logs em tempo real com auto-scroll thread-safe (`LogEmitter`), atualização de progresso de leitura 100% thread-safe (`StatsEmitter` com `pyqtSignal`), e aba de Relatórios com histórico e reprocessamento direto.
