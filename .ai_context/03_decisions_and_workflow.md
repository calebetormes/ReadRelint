...
- **[ADR-022] Especialidades Polimórficas (Homicídios):** Criação de modelos Pydantic estendidos (`HomicideReport` herdando de `IncidentReport`) para suportar dados especializados (motivação, registro policial, unidade BPM) sem alterar a estrutura da tabela SQLite, aproveitando a coluna de dados flexível em JSON e desserialização polimórfica pelo `bm_group`.
...
- **[ADR-024] Classificação Determinística por Regex (`bm_classifier.py`):** Adição de uma camada de segurança determinística pós-LLM que classifica o `bm_group` baseado em padrões de texto no nome do arquivo, assunto e conteúdo ordenados por especificidade (Homicídio > Tráfico > Roubos > Furtos > Outros), impedindo a classificação incorreta de fatos graves como "Outros".
- **[ADR-025] Adoção do Resend Design System (`Design.md`):** Reconstrução do tema CSS da aplicação Web com base nas diretrizes do `Design.md`: Canvas preto puro (`#000000`), hairlines translúcidas (`rgba(255,255,255,0.14)`), fontes Inter/Mono, eliminação de sombras pesadas e remoção de inline styles com cores claras hardcoded.
