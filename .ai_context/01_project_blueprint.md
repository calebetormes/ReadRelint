...
- **Painel de Controle:** Oferece um dashboard interativo (Streamlit) para buscar, cruzar vínculos de participantes, exibir dossiês, fotos e permitir a edição de dados extraídos pela IA.

### Regras de Ouro (Core Principles)
1. **Privacidade e Segurança:** NENHUM dado pode ser enviado para a nuvem. O processamento é 100% offline.
...
- **Interfaces:** `CustomTkinter` (Monitoramento/Desktop) e `Streamlit` (Dashboard de Curadoria).
...
- Dados Brutos: `images` (lista de ditos com path/caption), `content` (texto literal), `summary`, `user_edited`
...
*   `src/application/`: Orquestração do Pipeline (`etl_service.py`, `text_cleaner.py`).
*   `src/presentation/`:
    *   `/desktop/`: CustomTkinter UI (MainController, FolderWatcher, Tabs).
    *   `/web_dashboard/`: Streamlit (`dashboard_app.py`, `helpers.py`, UI master-detail).
