  ...
  detailPane.innerHTML = `
    <div class="detail-header">
      ...
      <div class="detail-actions">
        <button class="btn btn-secondary" onclick="openEditRelintModal('${relint.id}')">
          <i data-lucide="edit-3"></i> Revisar
        </button>
      </div>
    </div>
...
/* ─────────────────────────────────────────────────────────────
   Edit RELINT Modal Controllers
   ───────────────────────────────────────────────────────────── */
let currentEditingRelint = null;
...
async function openEditRelintModal(reportId) {
  ...
  try {
    const res = await fetch(`/api/v1/relints/${reportId}`);
    if (!res.ok) throw new Error('Erro ao carregar dados do RELINT para edição.');
    const relint = await res.json();
    currentEditingRelint = relint;

    document.getElementById('edit-relint-id').value = relint.id;
    document.getElementById('edit-modal-badge').textContent = extractRelintNumber(relint);
    ...
    document.getElementById('edit-relint-type').value = relint.relint_type || 'Outros';
    ...
    document.getElementById('edit-street').value = relint.street || '';
    document.getElementById('edit-number').value = relint.number || '';
    ...
    const isHomicide = (relint.bm_group || '').toLowerCase().includes('homicíd') || (relint.bm_group || '').toLowerCase().includes('homicid');
    const homicideTabBtn = document.getElementById('modal-tab-homicide-btn');
    if (homicideTabBtn) {
      homicideTabBtn.style.display = isHomicide ? 'inline-block' : 'none';
    }
    document.getElementById('edit-registry-number').value = relint.registry_number || '';
    document.getElementById('edit-registry-agency').value = relint.registry_agency || '';
    document.getElementById('edit-registry-year').value = relint.registry_year || '';
    document.getElementById('edit-fact-type').value = relint.fact_type || '';
    document.getElementById('edit-police-unit').value = relint.police_unit || '';
    document.getElementById('edit-motivation').value = relint.motivation || 'Desconhecido';

    // Populate Participants List
    const partContainer = document.getElementById('edit-participants-list');
    partContainer.innerHTML = '';
    const participants = relint.participants || [];
    participants.forEach(p => addParticipantRow(p));

    switchModalTab('edit-tab-general');
...
function closeEditRelintModal(e) {
...
function switchModalTab(tabId) {
  ...
  const activeBtn = Array.from(document.querySelectorAll('.modal-tab-btn')).find(btn => btn.getAttribute('onclick')?.includes(tabId));
  if (activeBtn) activeBtn.classList.add('active');
}

function addParticipantRow(p = {}) {
  const container = document.getElementById('edit-participants-list');
  if (!container) return;

  const row = document.createElement('div');
  row.className = 'edit-participant-row';
  row.innerHTML = `
    <input type="text" class="form-control part-name" placeholder="Nome completo" value="${escapeHtml(p.name || '')}" />
    <input type="text" class="form-control part-nickname" placeholder="Alcunha/Vulgo" value="${escapeHtml(p.nickname || '')}" />
    <input type="text" class="form-control part-doc" placeholder="CPF/RG" value="${escapeHtml(p.document || '')}" />
    <select class="form-control part-type">
      <option value="Acusado" ${p.participation_type === 'Acusado' ? 'selected' : ''}>Acusado</option>
      <option value="Suspeito" ${p.participation_type === 'Suspeito' ? 'selected' : ''}>Suspeito</option>
      <option value="Vítima" ${p.participation_type === 'Vítima' ? 'selected' : ''}>Vítima</option>
      <option value="Testemunha" ${p.participation_type === 'Testemunha' ? 'selected' : ''}>Testemunha</option>
      <option value="Parte da Guarnição" ${p.participation_type === 'Parte da Guarnição' ? 'selected' : ''}>Parte da Guarnição</option>
    </select>
    <button type="button" class="btn-remove-part" onclick="removeParticipantRow(this)" title="Remover participante">
      &times;
    </button>
  `;
...
async function saveRelintEdits() {
  const reportId = document.getElementById('edit-relint-id').value;
  if (!reportId) return;

  // Gather Participants
  const participantRows = document.querySelectorAll('.edit-participant-row');
  const participants = [];
  participantRows.forEach(row => {
    ...
    if (name) {
      participants.push({
        ...
        document: row.querySelector('.part-doc').value.trim(),
  ...
  const payload = {
    ...
    date_of_fact: document.getElementById('edit-date-of-fact').value.trim(),
    time_of_fact: document.getElementById('edit-time-of-fact').value.trim(),
    bm_group: document.getElementById('edit-bm-group').value,
    relint_type: document.getElementById('edit-relint-type').value,
    main_fact: document.getElementById('edit-main-fact').value.trim(),
    ...
    coordinates: document.getElementById('edit-coordinates').value.trim(),
    map_url: document.getElementById('edit-map-url').value.trim(),
    participants: participants,
    registry_number: document.getElementById('edit-registry-number').value.trim(),
    registry_agency: document.getElementById('edit-registry-agency').value.trim(),
    registry_year: document.getElementById('edit-registry-year').value.trim(),
    fact_type: document.getElementById('edit-fact-type').value.trim(),
    police_unit: document.getElementById('edit-police-unit').value.trim(),
  ...
  try {
    ...
    if (!res.ok) throw new Error('Falha ao salvar as edições do RELINT.');
    const updatedRelint = await res.json();

    // Close Modal
    closeEditRelintModal();

    // Refresh Local View & Master List
    selectRelint(reportId);
    if (typeof fetchRelintsList === 'function') fetchRelintsList();
    if (typeof fetchHomicides === 'function') fetchHomicides();
