...
function addParticipantEditRow(data = {}, relintImages = []) {
  ...
  row.innerHTML = `
    ...
    <select class="form-control part-type">
      <option value="Acusado" ${(data.participation_type === 'Acusado' || !data.participation_type) ? 'selected' : ''}>Acusado</option>
      ...
      <option value="Suspeito" ${data.participation_type === 'Suspeito' ? 'selected' : ''}>Suspeito</option>
    </select>
